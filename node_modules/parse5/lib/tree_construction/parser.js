...
Parser.prototype.parseFragment = function (html, fragmentContext) {
    //NOTE:1160 use <template> element as a fragment context if context element was not provided,
    //so we will parse in "forgiving" manner
    if (!fragmentContext)
        fragmentContext = this.treeAdapter.createElement($.TEMPLATE, NS.HTML, []);

    //NOTE:710 create fake element which will be used as 'document' for fragment parsing.
...
function aaInnerLoop(p, furthestBlock, formattingElement) {
    ...
    for (var i = 0; i < AA_INNER_LOOP_ITER; i++) {
        ...
        //NOTE:1070 store next element for the next loop iteration (it may be deleted from the stack by step 9.5)
...
function startTagInHead(p, token) {
    ...
    //NOTE:840 here we assume that we always act as an interactive user agent with enabled scripting, so we parse
...
function preStartTagInBody(p, token) {
    ...
    p._insertElement(token, NS.HTML);
    //NOTE:290 If the next token is a U+000A LINE FEED (LF) character token, then ignore that token and move
...
function textareaStartTagInBody(p, token) {
    p._insertElement(token, NS.HTML);
    //NOTE:300 If the next token is a U+000A LINE FEED (LF) character token, then ignore that token and move
...
//NOTE:850 here we assume that we always act as an user agent with enabled plugins, so we parse
...
function tableStartTagInTable(p, token) {
    ...
    //NOTE:410 The fake end tag token here can only be ignored in the fragment case.
    if (!fakeToken.ignored)
        p._processToken(token);
}
...
function startTagInCaption(p, token) {
    ...
    if (tn === $.CAPTION || tn === $.COL || tn === $.COLGROUP || tn === $.TBODY ||
        ...
        //NOTE:420 The fake end tag token here can only be ignored in the fragment case.
        if (!fakeToken.ignored)
            p._processToken(token);
    }
...
function endTagInCaption(p, token) {
    ...
    if (tn === $.CAPTION) {
        ...
        //NOTE:430 The fake end tag token here can only be ignored in the fragment case.
        if (!fakeToken.ignored)
            p._processToken(token);
    }
...
function tokenInColumnGroup(p, token) {
    ...
    //NOTE:440 The fake end tag token here can only be ignored in the fragment case.
    if (!fakeToken.ignored)
        p._processToken(token);
}
...
function startTagInRow(p, token) {
    ...
    if (tn === $.TH || tn === $.TD) {
        ...
        //NOTE:450 The fake end tag token here can only be ignored in the fragment case.
        if (!fakeToken.ignored)
            p._processToken(token);
    }
...
function endTagInRow(p, token) {
    ...
    if (tn === $.TR) {
        ...
        //NOTE:460 The fake end tag token here can only be ignored in the fragment case.
        if (!fakeToken.ignored)
            p._processToken(token);
    }
