...
exports.assign = function (tokenizer) {
    //NOTE:930 obtain Tokenizer proto this way to avoid module circular references
    var tokenizerProto = Object.getPrototypeOf(tokenizer);

    tokenizer.tokenStartLoc = -1;

    //NOTE:600 add location info builder method
    ...
    //NOTE:1000 patch token creation methods and attach location objects
    ...
    //NOTE:1010 patch token emission methods to determine end location
    tokenizer._emitCurrentToken = function () {
        //NOTE:900 if we have pending character token make it's end location equal to the
    ...
    tokenizer._emitCurrentCharacterToken = function () {
        //NOTE:880 if we have character token and it's location wasn't set in the _emitCurrentToken(),
        //then set it's location at the current preprocessor position
        if (this.currentCharacterToken && this.currentCharacterToken.location.end === -1) {
            //NOTE:1240 we don't need to increment preprocessor position, since character token
    ...
    //NOTE:980 patch initial states for each mode to obtain token start position
