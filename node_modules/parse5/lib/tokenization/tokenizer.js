...
//NOTE:400 String.fromCharCode() function can handle only characters from BMP subset.
...
Tokenizer.prototype._emitCurrentToken = function () {
    ...
    //NOTE:1050 store emited start tag's tagName to determine is the following end tag token is appropriate.
...
//NOTE:1170 used then we emit character explicitly. This is always a non-whitespace and a non-null character.
...
Tokenizer.prototype._consumeNamedEntity = function (startCp, inAttr) {
    ...
    for (; leaf && cp !== $.EOF; cp = this._consume(), consumedCount++, leaf = leaf.l && leaf.l[cp]) {
        if (leaf.c) {
            //NOTE:1270 we have at least one named reference match. But we don't stop lookup at this point,
    ...
    if (referencedCodePoints) {
        if (!semicolonTerminated) {
            //NOTE:1150 unconsume excess (e.g. 'it' in '&notit')
            this._unconsumeSeveral(consumedCount - entityCodePointsCount);

            //NOTE:280 If the character reference is being consumed as part of an attribute and the next character
...
Tokenizer.prototype._consumeCharacterReference = function (startCp, inAttr) {
    if (this.disableEntitiesDecoding || isWhitespace(startCp) || startCp === $.GREATER_THAN_SIGN ||
        ...
        //NOTE:910 not a character reference. No characters are consumed, and nothing is returned.
        this._unconsume();
        return null;
    }

    else if (startCp === $.NUMBER_SIGN) {
        //NOTE:1250 we have a numeric entity candidate, now we should determine if it's hex or decimal
        var isHex = false,
            nextCp = this._lookahead();
        ...
        //NOTE:870 if we have at least one digit this is a numeric entity for sure, so we consume it
        if (nextCp !== $.EOF && isDigit(nextCp, isHex))
            return [this._consumeNumericEntity(isHex)];

        else {
            //NOTE:950 otherwise this is a bogus number entity and a parse error. Unconsume the number sign
...
_[TAG_OPEN_STATE] = function tagOpenState(cp) {
    if (cp === $.EXCLAMATION_MARK)
        ...
        //NOTE:660 call bogus comment state directly with current consumed character to avoid unnecessary reconsumption.
...
_[END_TAG_OPEN_STATE] = function endTagOpenState(cp) {
    if (isAsciiUpper(cp)) {
        ...
        //NOTE:670 call bogus comment state directly with current consumed character to avoid unnecessary reconsumption.
        this[BOGUS_COMMENT_STATE](cp);
    }
};
...
_[MARKUP_DECLARATION_OPEN_STATE] = function markupDeclarationOpenState(cp) {
    if (this._consumeSubsequentIfMatch($$.DASH_DASH_STRING, cp, true)) {
        ...
        //NOTE:680 call bogus comment state directly with current consumed character to avoid unnecessary reconsumption.
        this[BOGUS_COMMENT_STATE](cp);
    }
};
